...
#         self.cv4 = nn.Sequential(Conv(ch[1], c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.nm, 1))
#
#     def forward(self, x):
...
#         mc = self.cv4(x[1]).view(bs, self.nm, -1)
#         x = Detect.forward(self, [x[1]])
#
#         # # 打印 mc 的形状
...
class Segment(Detect):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=()):
        ...
        c4 = max(ch[0] // 4, self.nm)
        #self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.nm, 1)) for x in ch)
        # self.cv41 = nn.Sequential(Conv(ch[0], c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.nm, 1))
        self.cv42 = nn.Sequential(Conv(ch[1], c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.nm, 1))
    ...
    def forward(self, x):
        ...
        # mc = torch.cat([self.cv4[i](x[i]).view(bs, self.nm, -1) for i in range(self.nl)], 2)  # mask coefficients
        # mc = torch.cat([self.cv41(x[0]).view(bs, self.nm, -1), self.cv42(x[1]).view(bs, self.nm, -1)], dim=2)
        mc = self.cv42(x[1]).view(bs, self.nm, -1)
        # print("x:",x.dim())
